...
(function () {
  ...
  const pieces = [
    ...
    {
      id: "cf104",
      client: "Restaurant · South Loop",
      title: "Chicago Firehouse",
      hero: HERO + "cf104.jpg",
      desc:
        "The CF·104 crest of the Chicago Firehouse Restaurant — a 1905 engine house in the South Loop that's now a city landmark, and where scenes from the film Backdraft were shot. The interlocking letters and dotted ·104· ring nod to the building's engine-company roots, carved with a ring thin enough to glow but thick enough to hold.",
    ...
    },
    {
      ...
      desc:
        "Michael Jordan's Steakhouse, carved from the restaurant's current logo — the name set as a tight modern wordmark, not the old autograph. The strokes are kept continuous so the mark reads instantly, even before it's lit.",
      tags: ["Wordmark", "Modern logo", "Steakhouse"],
      stages: [{ l: "Lit", img: "hero/mjordan.jpg" }],
      note:
        "Michael Jordan's Steakhouse — carved from the restaurant's current wordmark logo rather than the old signature.",
    },
    {
      id: "kinmont",
      pair: true,
    ...
    {
      id: "macallan",
      client: "For a co-worker's dog",
      title: "Macallan",
      hero: HERO + "macallan.jpg",
      desc:
        "A whisky-label parody for a co-worker's golden doodle named Macallan, after the scotch. 'Golden Doodle Release' and '10 Doggy Years' replace the age statement, the dog sketched in where the distillery would sit — relief-carved by day, glowing single-malt amber once it's lit.",
      tags: ["Whisky label", "Parody", "For a friend"],
      stages: [
        { l: "Carved on the bench", img: "hero/macallan-carved.jpg" },
        { l: "Lit", img: "hero/macallan.jpg" },
      ],
      note:
        "Macallan the golden doodle, named after the scotch — so the carve became a mock whisky label, 'Golden Doodle Release · 10 Doggy Years', for a co-worker.",
    },
    {
      id: "applebees",
  ...
  pieces.forEach((p, i) => {
    ...
    const candle = `<span class="link-candle" aria-hidden="true"><i class="lc-flame"></i><i class="lc-stick"></i></span>`;
    ...
    if (p.pair) {
      ...
      <div class="pair-info">
        ...
        <a class="piece-link" data-id="${p.id}" href="#">More shots ${candle}</a>
      </div>`;
    } else {
